...
often one line of a hundred instructions. `n` steps over a whole loop, `o` steps
out of one, and `w` watches a cell. There is a
[thirteen-lesson tutorial](docs/tutorial.md) built on the same machinery, which
records every step so you can walk *backwards* through a loop.
